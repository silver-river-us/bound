...
func Parse(r io.Reader) (*model.Architecture, error) {
	...
	for scanner.Scan() {
		...
		if architecture == nil {
			...
			architecture = &model.Architecture{Name: match[1], Description: pendingDescription, Contexts: map[string]*model.Context{}, Objects: map[string]*model.Object{}, Modules: map[string]*model.Module{}, Span: model.Span{Line: lineNumber, Column: 1}}
			pendingDescription = ""
			continue
		}
		...
		if current == nil {
			if architecture.Implementation.Language == "" {
				...
				architecture.ImplementationSpan = model.Span{Line: lineNumber, Column: 1}
				continue
			}
			...
			if match := domainRE.FindStringSubmatch(line); match != nil {
				...
				currentObject = &model.Object{Name: match[2], Kind: match[1], Description: pendingDescription, Attributes: map[string]model.Attribute{}, Operations: map[string]model.Operation{}, Span: model.Span{Line: lineNumber, Column: 1}}
				pendingDescription = ""
			...
			if match := contextRE.FindStringSubmatch(line); match != nil {
				...
				current = &model.Context{Name: match[1], Description: pendingDescription, Exposes: map[string]bool{}, ExposeSpans: map[string]model.Span{}, Interfaces: map[string]*model.Interface{}, Modules: map[string]*model.Module{}, Span: model.Span{Line: lineNumber, Column: 1}}
				pendingDescription = ""
				architecture.Contexts[current.Name] = current
				continue
			}
			if match := relationRE.FindStringSubmatch(line); match != nil {
				architecture.Relations = append(architecture.Relations, model.Relation{From: match[1], To: match[2], Via: match[3], Description: pendingDescription, Span: model.Span{Line: lineNumber, Column: 1}})
				pendingDescription = ""
				continue
			}
		...
		if len(moduleStack) > 0 {
			...
			if match := moduleRE.FindStringSubmatch(line); match != nil {
				...
				module.Span = model.Span{Line: lineNumber, Column: 1}
				child.Span = model.Span{Line: lineNumber, Column: 1}
		...
		case exposesRE.MatchString(line):
			exposed := exposesRE.FindStringSubmatch(line)[1]
			current.Exposes[exposed] = true
			current.ExposeSpans[exposed] = model.Span{Line: lineNumber, Column: 1}
		case interfaceRE.MatchString(line):
			...
			currentInterface = &model.Interface{Name: match[1], Description: pendingDescription, Types: map[string]*model.Object{}, Operations: map[string]model.Operation{}, Span: model.Span{Line: lineNumber, Column: 1}}
			pendingDescription = ""
	...
	if insideQualityRules {
		return nil, &Error{Line: lineNumber, Column: 1, Message: "unclosed quality rules"}
	}
	if insideQuality {
		return nil, &Error{Line: lineNumber, Column: 1, Message: "unclosed quality policy"}
	}
	if currentObject != nil {
		return nil, &Error{Line: lineNumber, Column: 1, Message: fmt.Sprintf("unclosed domain type %s", currentObject.Name)}
	}
	if len(moduleStack) > 0 {
		return nil, &Error{Line: lineNumber, Column: 1, Message: fmt.Sprintf("unclosed module %s", moduleStack[len(moduleStack)-1].Qualified)}
	}
	if current != nil {
		if currentInterface != nil {
			return nil, &Error{Line: lineNumber, Column: 1, Message: fmt.Sprintf("unclosed interface %s", currentInterface.Name)}
		}
		return nil, &Error{Line: lineNumber, Column: 1, Message: fmt.Sprintf("unclosed context %s", current.Name)}
	}
	return nil, &Error{Line: lineNumber, Column: 1, Message: fmt.Sprintf("unclosed architecture %s", architecture.Name)}
}
...
func ParseMap(r io.Reader) (string, []model.FileMapping, error) {
	...
	for scanner.Scan() {
		...
		if match := fileRE.FindStringSubmatch(line); match != nil {
			files = append(files, model.FileMapping{Path: match[1], Node: match[2], Explicit: true})
			continue
		}
...
func syntaxError(line int, message string) error {
	return &Error{Line: line, Column: 1, Message: message}
}
